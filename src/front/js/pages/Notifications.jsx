        ...
        <div className="trips-container">
            ...
            {!hostRequests || hostRequests.length === 0 ? (
              <div className="no-trips-message">
                <p>You don't have any pending requests. Please come back later!</p>
              </div>
            ) : (
                ...
                {hostRequests.map((request, index) => (
                    <ul className="list-group mb-3" key={index}>
                        <li className="list-group-item">
                            <p>The user {request.traveler?.first_name} {request.traveler?.last_name} 
                                 has requested to join your trip to {request.trip?.destination}
                                <button className="btn btn-success me-2" onClick={() => handleApproveTrip(request.trip_id, request.traveler_id)}>
                                Approve
                            </button>
                            <button className="btn btn-danger" onClick={() => handleDeclineTrip(request.trip_id, request.traveler_id)}>
                                Decline
                            </button>
                            </p>
                        </li>
